  ...
  `{D, {    \  :    \;
    ...
    \;'   /  ,' /  _  \  /  _  \   ,'/
          ...
          \   `'  / \  `'  / \  `.' /
           ...
           `.___,'   `.__,'   `.__,'  Instagram Profile Spam Bot


------------------------------------------------

Coded By CoderPy
instagram: coderrpy , algospoines
"""
...
class Main:
    ...
    def login(self):
        ...
        if json_data["authenticated"]:
            print(f"Giriş yapıldı ({self.username})")
            ...
            print(f"""Giriş yapılamadı ({self.username}) \n 
İnstagram hesabına girip çıkan uyarıya bu bendim dersen hata düzelir.""")
    ...
    def spam(self):
        if self.log == True:
            ...
            spamPostResponse = self.ses.post("https://www.instagram.com/users/" + self.spamUserId + "/report/",
                                             data=spamData)

            if spamPostResponse.status_code == 200:
                self.ses.close();
                return 1
            else:
                return 0
...
if __name__ == "__main__":
    ...
    spamUser = input(Fore.GREEN + "SPAM'lanacak Kişinin Kullanıcı Adı: " + Style.RESET_ALL)
    spamUserId = input(Fore.GREEN + "SPAM'lanacak Kişinin User ID'si: " + Style.RESET_ALL)
    print("")
